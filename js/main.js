...
async function handleGoogleSignIn(response) {
    ...
    try {
      ...
      if (result.ok) {
        ...
        localStorage.setItem('siga_admin_email', result.admin.email);
...
function addSettingsRubroRow(nombre = '', precioNino = '', precioNina = '', descuentoHermano = '0') {
  ...
  row.innerHTML = 
    ...
    <input type="number" class="sr-nino" placeholder="Costo Ni�o �" value="${precioNino}" style="padding:8px; border-radius:6px; border:1px solid #cbd5e1; font-size:13px;" required>
    <input type="number" class="sr-nina" placeholder="Costo Ni�a �" value="${precioNina}" style="padding:8px; border-radius:6px; border:1px solid #cbd5e1; font-size:13px;" required>
...
function openModal(num) {
  ...
  // Tarjeta de saldo a favor (excedente)
    ...
    let btnCuotaMain = document.getElementById('btnCuotaMainFicha');
    if (isAdminMode) {
      if (!btnCuotaMain) {
        btnCuotaMain = document.createElement('button');
        btnCuotaMain.id = 'btnCuotaMainFicha';
        btnCuotaMain.className = 'btn btn-sm';
        btnCuotaMain.style.cssText = 'background:#f59e0b; color:white; padding:4px 8px; font-size:11px; margin-left:10px; border-radius:4px; font-weight:bold; cursor:pointer; border:none;';
        btnCuotaMain.innerHTML = '⭐ Cuota Especial';
        
        // Insert after modal-name
        const mName = document.getElementById('m-name');
        mName.parentNode.insertBefore(btnCuotaMain, mName.nextSibling);
      }
      btnCuotaMain.onclick = () => {
         closeModal();
         openCuotaModal(s.id, s.nombre);
      };
      btnCuotaMain.style.display = 'inline-block';
    } else if (btnCuotaMain) {
      btnCuotaMain.style.display = 'none';
    }
  ...
  var drawRest = function() {
    ...
    var details = [
      ...
      { label: 'Fecha de Registro:', val: tx.fecha },
        { label: 'Fecha de Transacción:', val: (tx.analisis?.fechaExtraida || tx.fecha || 'N/A') },
...
async function deleteEgreso(id) {
  if (!confirm('AEliminar este egreso?')) return;
  try {
...
function loadStudentsAdmin() {
  ...
  const seccion = currentSeccion || '1-1';
  const anio = currentAnio || '2027';
  const actId = currentActividadId;
  
  fetch(SCRIPT_URL + '/data?seccion=' + encodeURIComponent(seccion) + '&anio=' + encodeURIComponent(anio) + (actId ? '&actividadId='+actId : ''))
    .then(r => r.json())
    .then(res => {
      if(res.ok) {
...
function renderAdminStudents() {
  ...
  if(adminStudentsList.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:15px;">No hay estudiantes en esta sección</td></tr>';
    return;
  }
  
  adminStudentsList.forEach((st, index) => {
    let genLabel = st.genero === 'Niño' ? '👦 Niño' : (st.genero === 'Niña' ? '👧 Niña' : st.genero);
    let hermLabel = st.hermanos ? '✅ Sí' : '❌ No';
    ...
    tr.innerHTML = `
      ...
      <td style="padding:10px; text-align:center;">
         <button class="btn btn-sm" style="background:#3b82f6; color:white; padding:4px 8px; font-size:11px;" onclick="editStudent('${st.id}')">✏️ Editar</button>
         <button class="btn btn-sm" style="background:#ef4444; color:white; padding:4px 8px; font-size:11px;" onclick="deleteStudent('${st.id}', '${st.nombre.replace(/'/g, "\'")}')">🗑️ Baja</button>
      </td>
    `;
...
function openStudentModal() {
  ...
  document.getElementById('studGenero').value = 'Niño';
  ...
  document.getElementById('studentModalTitle').textContent = 'Añadir Estudiante';
  document.getElementById('studentModalTitle').style.color = '#10b981';
  
  // Ocultar botón de cuota especial porque es un estudiante nuevo
  const btnCuota = document.getElementById('btnOpenCuotaFromStudent');
  if (btnCuota) {
...
function editStudent(id) {
  const st = adminStudentsList.find(s => String(s.id) === String(id));
  if(!st) return;
  ...
  if(st.fechaNacimiento && st.fechaNacimiento.includes('/')) {
    ...
    if(parts.length === 3) dVal = parts[2] + '-' + parts[1] + '-' + parts[0];
  } else {
    dVal = st.fechaNacimiento || '';
  }
  document.getElementById('studNacimiento').value = dVal;
  
  document.getElementById('studGenero').value = st.genero || 'Niño';
  document.getElementById('studHermano').checked = st.hermanos === true;
  ...
  document.getElementById('studentModalTitle').style.color = '#3b82f6';
  
  // Mostrar botón de cuota especial porque estamos editando
  const btnCuota = document.getElementById('btnOpenCuotaFromStudent');
  if (btnCuota) {
...
async function saveStudent() {
  ...
  const payload = {
    ...
    seccion: currentSeccion || '1-1',
    anio: currentAnio || '2027',
...
async function deleteStudent(id, nombre) {
    const result = await Swal.fire({
      title: '¿Dar de baja?',
      text: `¿Seguro que deseas dar de baja y eliminar a ${nombre}? Esta acción es permanente y reordenará la lista.`,
      icon: 'warning',
...
function openCuotaFromStudent() {
  const studId = document.getElementById('studId').value;
  const studNombre = document.getElementById('studNombre').value;
  if (!studId) return;
...
async function loadSeccionesForEdit() {
  ...
  try {
    ...
    if(json.ok) {
      ...
      select.innerHTML = '<option value="">-- Crear una Nueva Secci�n --</option>';
      allSeccionesConfig.forEach(s => {
        const opt = document.createElement('option');
        opt.value = s.seccion;
        opt.textContent = 'Secci�n ' + s.seccion + ' (' + s.docente_nombre + ')';
        select.appendChild(opt);
      });
    }
  } catch(e) {
